...
def getjwt()
  #files = ['./privatekey.pem']
  files = ['c:\temp\privatekey.pem']
  certs = files.map { |f| File.open(f).read }
  privatekey = OpenSSL::PKey::RSA.new(certs[0])
  ...
  payload = {
    jti: SecureRandom.uuid, # random string
  ...
  jwttoken = JWT.encode payload, privatekey, 'RS256', header
  return jwttoken
end
...
server.mount_proc('/') do |req, res|
  ...
  res.body = html.sub("{jwttoken}", getjwt())
end
